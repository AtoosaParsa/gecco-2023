...
def MD_VibrSP_ConstV_Yfixed_DiffK(stiffness, B, Nt, N, x_ini, y_ini, D0, m0, L, Freq_Vibr, Amp_Vibr, ind_s, Amp_Vibr1, ind_in1, Amp_Vibr2, ind_in2, ind_out, noise):
    ...
    sig_avg_watts = np.mean(Amp_Vibr1*np.sin(Freq_Vibr*dt*np.arange(Nt)))
    ...
    mean_noise = 0
    noise_volts = np.random.normal(mean_noise, np.sqrt(noise_avg_watts), len(x_in1))
    # Noise up the original signal
    x_in1 =x_in1 + noise_volts

    # Set a target SNR
    target_snr_db = noise
    # Calculate signal power and convert to dB 
    sig_avg_watts = np.mean(Amp_Vibr2*np.sin(Freq_Vibr*dt*np.arange(Nt)))
    sig_avg_db = 10 * np.log10(sig_avg_watts)
    # Calculate noise according to [2] then convert to watts
    noise_avg_db = sig_avg_db - target_snr_db
    noise_avg_watts = 10 ** (noise_avg_db / 10)
    # Generate an sample of white noise
    mean_noise = 0
    noise_volts = np.random.normal(mean_noise, np.sqrt(noise_avg_watts), len(x_in2))
    # Noise up the original signal
    x_in2 =x_in2 + noise_volts
    ...
    if mark_vibrY == 0:
        freq_fft, fft_in1 = FFT_Fup(int(Nt/2), x_in1[int(Nt/2):Nt], dt, Freq_Vibr)
        freq_fft, fft_in2 = FFT_Fup(int(Nt/2), x_in2[int(Nt/2):Nt], dt, Freq_Vibr)
        freq_fft, fft_s = FFT_Fup(int(Nt/2), x_s[int(Nt/2):Nt], dt, Freq_Vibr)   
    elif mark_vibrY == 1:
        freq_fft, fft_in1 = FFT_Fup(int(Nt/2), y_in1[int(Nt/2):Nt], dt, Freq_Vibr)
        freq_fft, fft_in2 = FFT_Fup(int(Nt/2), y_in2[int(Nt/2):Nt], dt, Freq_Vibr)
        freq_fft, fft_s = FFT_Fup(int(Nt/2), y_s[int(Nt/2):Nt], dt, Freq_Vibr)
    ...
    return freq_fft, fft_in1, fft_in2, fft_s, fft_x_out, fft_y_out, np.mean(cont), nt_rec, Ek_now, Ep_now, cont_now

def MD_VibrSP_ConstV_Yfixed_DiffK2(stiffness, B, Nt, N, x_ini, y_ini, D0, m0, L,  Freq_Vibr, Amp_Vibr, ind_s, Amp_Vibr1, ind_in1, Amp_Vibr2, ind_in2, ind_out, noise):
