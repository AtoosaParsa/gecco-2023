...
def MD_VibrSP_ConstV_Yfixed_DiffK(stiffness, B, Nt, N, x_ini, y_ini, D0, m0, L, Freq_Vibr, Amp_Vibr, ind_s, Amp_Vibr1, ind_in1, Amp_Vibr2, ind_in2, ind_out, noise):
    ...
    if mark_vibrY == 0:
        x_in1 = Amp_Vibr1*np.sin(Freq_Vibr*dt*np.arange(Nt))+x_ini[ind_in1] + np.random.normal(0, noise*Amp_Vibr1, Nt)
        x_in2 = Amp_Vibr2*np.sin(Freq_Vibr*dt*np.arange(Nt))+x_ini[ind_in2] + np.random.normal(0, noise*Amp_Vibr2, Nt)
