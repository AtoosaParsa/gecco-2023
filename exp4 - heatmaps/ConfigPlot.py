...
def ConfigPlot_DiffStiffness3(N, x, y, D, L, stiffness, mark_print, hn, in1, in2, so, out):
        ...
        for k in range(-1, 2):
            for l in range(-1, 2):                        
                ...
                if i==in1:
                    ...
                    plt.scatter(x_now, y_now, marker='o', s=300, color=(1, 0, 1, 1))
                    
                e.set_zorder(20)
                ells.append(e)
                k_all.append(stiffness[i])
    ...
    for e in ells:
        ...
        e.set_clip_box(ax.bbox)
        e.set_facecolor('k')
        e.set_alpha((k_all[i]-k_min)/(k_max-k_min))
        e.set_zorder(0)
    ...
    black_circle = mlines.Line2D([], [], color=(1, 0, 1), marker='o', linestyle='None',
                            markersize=15, label='Source')
    plt.legend(handles=[red_star, green_triangle, blue_square, black_circle], bbox_to_anchor=(1.315, 1), fontsize=14)
...
def ConfigPlot_DiffStiffness4(N, x, y, D, L, stiffness, mark_print, hn, in1, in2, so, out, alphas, betas, label):
    ...
    color_list = cmap(alphas)

    ells = []
    alpha_all = []
    beta_all = []
    k_min = 1 #min(stiffness)
    k_max = 10 #max(stiffness)
    for i in range(N):
        x_now = x[i]%L[0]
        y_now = y[i]%L[1]
        for k in range(-1, 2):
            for l in range(-1, 2):                        
                e = Ellipse((x_now+k*L[0], y_now+l*L[1]), D[i],D[i],0)
                ells.append(e)
                alpha_all.append(color_list[i])#alphas[i]) #use nandness instead of stiffness as alpha: (stiffness[i]-k_min)/(k_max-k_min))
                beta_all.append(betas[i])

    i = 0
    for e in ells:
        ax.add_artist(e)
        e.set_clip_box(ax.bbox)
        e.set_facecolor(alpha_all[i]) #'k')
        e.set_edgecolor('orange')
        e.set_linewidth(1+20*beta_all[i])
        #e.set_alpha(1) #alpha_all[i])
        e.set_zorder(0)
        i += 1
    ...
    for i in range(N):
        ...
        for k in range(-1, 2):
            for l in range(-1, 2):                        
                ...
                #e.set_linewidth(1+20*beta_all[i])
                if i==in1:
                    plt.scatter(x_now, y_now, marker='^', s=1100, color=(0, 1, 0, 1))
                elif i==in2:
                    plt.scatter(x_now, y_now, marker='s', s=1100, color=(0, 0, 1, 1))
                elif i==out:
                    plt.scatter(x_now, y_now, marker='*', s=1100, color=(1, 0, 0, 1))
                elif i==so:
                    plt.scatter(x_now, y_now, marker='o', s=1100, color=(1, 0, 1, 1))
    ...
    # draw walls
    ax.plot([0, L[0]], [0, 0], color='black', linewidth=24, zorder=30)
    ax.plot([0, L[0]], [L[1], L[1]], color='black', linewidth=24, zorder=30)

    import matplotlib.lines as mlines
    red_star = mlines.Line2D([], [], color=(1, 0, 0), marker='*', linestyle='None',
                            markersize=40, label='Output')
    blue_square = mlines.Line2D([], [], color=(0, 0, 1), marker='s', linestyle='None',
                            markersize=40, label='Input 2')
    green_triangle = mlines.Line2D([], [], color=(0, 1, 0), marker='^', linestyle='None',
                            markersize=40, label='Input 1')
    black_circle = mlines.Line2D([], [], color=(1, 0, 1), marker='o', linestyle='None',
                            markersize=40, label='Source')
    plt.legend(loc='upper center', handles=[red_star, green_triangle, blue_square, black_circle], bbox_to_anchor=(0.5, -0.10), ncol=4, fontsize=24)
    plt.set_cmap("viridis")
    plt.clim(0, 1)
    cbar=plt.colorbar()
    cbar.ax.locator_params(nbins=5)
    cbar.set_label('NAND-ness($\omega$=10)', fontsize=42)
    cbar.ax.tick_params(labelsize=32, width=2, length=7)
    plt.xticks([])
    plt.yticks([])
    plt.show() 
    if mark_print == 1:
    ...
    fig, ax = plt.subplots(subplot_kw={'aspect': 'equal'})
    e = Ellipse((0.2, 0.15), D[0], D[0], 0)
    e.set_edgecolor('orange')
    e.set_facecolor('none')
    e.set_linewidth(1+20*min(beta_all))
    print(1+20*min(beta_all))
    ax.add_artist(e)
    e = Ellipse((0.2, 0.35), D[0], D[0], 0)
    e.set_edgecolor('orange')
    e.set_facecolor('none')
    e.set_linewidth(1+20*max(beta_all))
    print(1+20*max(beta_all))
    ax.add_artist(e)
    ax.set_xlim(0, L[0])
    ax.set_ylim(0, L[1])
    plt.show()
    ...
    count = 0
    print("betas")
    for i in np.arange(1+20*min(beta_all), 1+20*max(beta_all)+1):
        print(i)
        plt.plot([10, 10], [count, count+5], linewidth=i, color='orange')
        count = count+5
    ax.set_xlim(-10, count+10)
    ax.set_ylim(-10, count+10)
    plt.show()
    return fig

def ConfigPlot_DiffStiffness5(N, x, y, D, L, stiffness, mark_print, hn, in1, in2, so, out):
    ...
    for i in range(N):
        ...
        for k in range(-1, 2):
            for l in range(-1, 2):                        
                e = Ellipse((x_now+k*L[0], y_now+l*L[1]), D[i],D[i],0)
                ells.append(e)
                alpha_all.append((stiffness[i]-k_min)/(k_max-k_min))
    ...
    cbar=plt.colorbar()
    cbar.set_label("Stiffness", fontsize=24)
...
# plot colormap with greys
def ConfigPlot_DiffStiffness6(N, x, y, D, L, stiffness, mark_print, hn, in1, in2, so, out, alphas, label):
    ...
    for i in range(N):
        ...
        for k in range(-1, 2):
            for l in range(-1, 2):                        
                e = Ellipse((x_now+k*L[0], y_now+l*L[1]), D[i],D[i],0)
                ells.append(e)
                alpha_all.append(alphas[i]) #use nandness instead of stiffness as alpha: (stiffness[i]-k_min)/(k_max-k_min))
