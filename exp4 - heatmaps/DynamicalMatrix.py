...
def DM_mass_DiffK_Yfixed(N, x0, y0, D0, m0, Lx, y_bot, y_top, stiffness):
    ...
    for i in range(N):
        r_now = 0.5*D0[i]
        if y0[i]-y_bot<r_now or y_top-y0[i]<r_now:
            M[2*i+1, 2*i+1] = M[2*i+1, 2*i+1] + stiffness[i] / r_now / r_now
        for j in range(i):
            dij = 0.5 * (D0[i] + D0[j])
            dijsq = dij**2
            dx = x0[i] - x0[j]
            ...
            if rijsq < dijsq:
                ...
                if stiffness[i] != stiffness[j]:
                    k = (stiffness[i]*stiffness[j])/(stiffness[i]+stiffness[j])
                else:
                    k = stiffness[i]
                rijmat = np.array([[dx*dx, dx*dy], [dx*dy, dy*dy]])
                rij = np.sqrt(rijsq)
