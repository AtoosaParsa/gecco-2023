...
class AFPO:
    ...
    def __init__(self,randomSeed):        
        ...
        for populationPosition in range(c.popSize):
            ...
            self.genomes[populationPosition] = GENOME(self.nextAvailableID, self.nextSpecies)
            self.nextAvailableID = self.nextAvailableID + 1
            self.nextSpecies = self.nextSpecies + 1
...
# -------------------------- Private methods ----------------------
    ...
    def Expand(self):
        ...
        for newGenome in range( popSize , 2 * popSize - 1 ):
            ...
            # the child has the same id as its parent
            self.genomes[newGenome].Set_ID(self.nextAvailableID)
            self.genomes[newGenome].Set_species(self.genomes[spawner].indv.species)
            self.nextAvailableID = self.nextAvailableID + 1
    ...
    def Inject(self):
        popSize = len(self.genomes)
        self.genomes[popSize-1] = GENOME(self.nextAvailableID, self.nextSpecies)
        self.nextAvailableID = self.nextAvailableID + 1
        self.nextSpecies = self.nextSpecies + 1
    ...
    def SaveLastGen(self):
        genomesSortedByFitness = sorted(self.genomes.values(), key=operator.attrgetter('fitness'),reverse=False)
        f = open(f'savedRobotsLastGenAfpoSeed{self.randomSeed}.dat', 'ab')
        pickle.dump(genomesSortedByFitness, f)
        f.close()

    def SaveGen(self):
        filename = f'gens{self.randomSeed}/gen{self.currentGeneration}.dat'
        f = open(filename, 'ab')
        pickle.dump(self.genomes, f)
        f.close()

    def Save_Avg(self):
        f = open(f'avgFitnessAfpoSeed{self.randomSeed}.dat', 'ab')
